...
void moveForward(int dir) {
    // TODO: 请实现
    dequeue();
    int i = (rear - 1) % MAX_SIZE;
    enqueue(queueX[i]+dx[dir], queueY[i]+dy[dir]);
}
...
int decideDirection() {
    ...
    for (int d = 0; d < 4; d = d + 1) {
        ...
        bool optim = (abs(foodX-headX) > abs(foodX-nx)||abs(foodY-headY) > abs(foodY-ny));
...
// 第46行: 期望 ;，但得到 'Node'#include <deque>
// #include <stack>
...
//     int d;
//     Node* parent;
//     Node(int a, int b, int dir=0): x(a), y(b), d(dir), parent(NULL){}
// };


// bool in_queue(std::deque<Node*> path, int x, int y){
...
//     while(!in_queue(path, foodX, foodY)){
        
//         Node* elem = path.front();
//         path.pop_front();
//         searched_nodes.push_back(elem);
...
//             if (cell != 1 && cell != 3 && cell != 2 && checkSelfCollision(nx, ny) == 0 && !in_queue(path, nx, ny) && !in_queue(searched_nodes, nx, ny)) {
//                 Node* node = new Node(nx, ny, d);
...
//     for(int i = 0; i<path.size();++i){
//         if(path[i]->x == foodX && path[i]->y == foodY){
//             final = path[i];
//             break;
//         }
//     }
...
//     Node* p = final;
//     while(p->parent != head) p = p->parent; 
